...
REM ************************************
REM ** ジョブID　：NSBA140S010
REM ** ジョブ名　：日本触媒_支払データ抽出
REM ** 取引先名　：日本触媒
REM ** カテゴリ名：支払・支払明細
REM ** 引数　　　：なし
...
REM **    1.00 2025/07/10 FJ)松田 【R60795】新規作成
REM *********************************************************************************
...
REM 変数設定
REM =============================
SET JOBNAME01=NSBA140S010
SET JOBNAME02=NSBA140S020
SET PRM_FILE01=p_t_NSB_支払_010.param
SET PRM_FILE02=p_t_NSB_支払_020.param
...
call %IDMC_RUNAJOBCLI_INST_DIR%\cli.bat runAJobCli ^
  ...
  -w true ^
  -t TASKFLOW ^
  -un wf_NSB_A140_010 >> %JOBLOG_PATH%\%JOBLOG% 2>&1
...
call %IDMC_RUNAJOBCLI_INST_DIR%\cli.bat runAJobCli ^
  ...
  -w true ^
  -t TASKFLOW ^
  -un wf_NSB_A140_020 >> %JOBLOG_PATH%\%JOBLOG% 2>&1
