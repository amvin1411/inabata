...
SETLOCAL
REM ************************************
REM ** ジョブID　：NSBA130S010
REM ** ジョブ名　：日本触媒_請求確認データ抽出
REM ** 取引先名　：日本触媒
REM ** カテゴリ名：請求確認
REM ** 引数　　　：なし
...
REM **    1.00 2025/05/20 FJ)松枝 【R60795】新規作成
REM *********************************************************************************
...
REM 変数設定
REM =============================
SET JOBNAME01=NSBA130S010
SET JOBNAME02=NSBA130S020
SET PRM_FILE01=p_t_NSB_請求確認_010.param
SET PRM_FILE02=p_t_NSB_請求確認_020.param
...
REM =============================
:STEP000
ECHO %date% %time% ジョブID: %JOBNAME01% の実行開始 >> %JOBLOG_PATH%\%JOBLOG%
...
call %IDMC_RUNAJOBCLI_INST_DIR%\cli.bat runAJobCli ^
  ...
  -w true ^
  -t TASKFLOW ^
  -un wf_NSB_A130_010 >> %JOBLOG_PATH%\%JOBLOG% 2>&1
...
call %IDMC_RUNAJOBCLI_INST_DIR%\cli.bat runAJobCli ^
  ...
  -w true ^
  -t TASKFLOW ^
  -un wf_NSB_A130_020 >> %JOBLOG_PATH%\%JOBLOG% 2>&1
...
REM =============================
:NORMALEND
ECHO %date% %time% ジョブID: %JOBNAME01% 正常終了 >> %JOBLOG_PATH%\%JOBLOG%
SET JRC=0
GOTO END
...
ECHO %date% %time% ジョブID: %JOBNAME01% 異常終了 >> %JOBLOG_PATH%\%JOBLOG%
SET JRC=99
